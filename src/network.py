...
class Encoder(nn.Module):
    ...
    def __init__(self, num_hidden):
        ...
        self.encoder_prenet = Prenet(hp.n_mels, num_hidden * 2, num_hidden, p=0.2)
        # self.encoder_prenet = Prenet(num_hidden, num_hidden * 2, num_hidden, p=0.2)
    ...
    def forward(self, x, pos):
        ...
        # Encoder Pre-network
        x = self.encoder_prenet(x)  # 80 -> 256
        x = self.norm(x)
...
class MelDecoder(nn.Module):
    ...
    def forward(self, memory, decoder_input, c_mask, pos):
        ...
        # t.set_printoptions(edgeitems=1000)
...
class Model(nn.Module):
    ...
    def __init__(self):
        ...
        self.encoder = Encoder(hp.hidden_size)   # 256
        self.decoder = MelDecoder(hp.hidden_size)
        # self.encoder = Encoder(hp.n_mels)
        # self.decoder = MelDecoder(hp.n_mels)

    def forward(self, src, trg, pos_src, pos_trg):
        memory, c_mask, attns_enc = self.encoder.forward(src, pos_src)  # src 80 -> 256
        mel_output, postnet_output, attn_probs, attns_dec = self.decoder.forward(memory, trg, c_mask, pos_trg)  # trg 80 -> 256 -> 80

        return mel_output, postnet_output, attn_probs, attns_enc, attns_dec # output 80
